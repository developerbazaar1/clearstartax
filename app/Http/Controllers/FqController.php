    ...
    { 
        ...
        if ($action === 'function2') {
                ...
                if ($request->input('Marital_status') == 'Married') {
                    $rules['Married_Filing_Status'] = 'required';
                }

                if ($request->input('Married_Filing_Status') == 'Married Filing Jointly') {
                    $rules['spouse_first_name'] = 'required';
                    $rules['spouse_last_name'] = 'required';
                    $rules['spouse_Driver_License'] = 'required';
                    $rules['spouse_ssn'] = 'required';
                    $rules['spouse_date_of_birth'] = 'required';
                }

                if ($request->input('Do_you_have_any_dependents_EXCLUDING_yourself_and_your_spouse') == 'have any dependents yes') {
                    $rules['How_many_dependents_do_you_have'] = 'required';
                }



                if ($request->input('Client_employment_status') == 'Self-Employed') {
                    $rules['Business_Cash_On_Hand'] = 'required';
                    $rules['Business_Bank_Account'] = 'required';
                    $rules['Are_you_able_to_provide_current_year_Profit_Loss_Statement'] = 'required';
                }

                if ($request->input('Client_employment_status') == 'Self-Employed' && $request->input('Are_you_able_to_provide_current_year_Profit_Loss_Statement') == 'yes') {
                    $rules['Upload_Most_Recent_Profit_Loss_Statement'] = 'required';
                }

                if ($request->input('Client_employment_status') == 'Self-Employed' && $request->input('Are_you_able_to_provide_current_year_Profit_Loss_Statement') == 'no') {
                    $rules['Item_Actual_Monthly_Gross_Receipts'] = 'required';
                    $rules['Gross_Rental_Income'] = 'required';
                    $rules['Interest'] = 'required';
                    $rules['Dividends'] = 'required';
                    $rules['Cash'] = 'required';
                    $rules['Total_Business_Income'] = 'required';
                    $rules['Materials_Purchased'] = 'required';
                    $rules['Inventory_Purchased'] = 'required';
                    $rules['Gross_Wages_Salaries'] = 'required';
                    $rules['Rent'] = 'required';
                    $rules['Supplies'] = 'required';
                    $rules['Business_Expenses'] = 'required';
                    $rules['Utilities_Telephone'] = 'required';
                    $rules['Vehicle_Gas_Oil'] = 'required';
                    $rules['Repairs_Maintenance'] = 'required';
                    $rules['Insurance'] = 'required';
                    $rules['Current_Taxes'] = 'required';
                    $rules['Total_Business_Expenses'] = 'required';
                    $rules['Net_Business_Income'] = 'required';

                }


                if ($request->input('Are_you_currently_in_bankruptcy') == 'Yes') {
                    $rules['Discharge/Dismissal_Date'] = 'required';
                }

                if ($request->input('Have_you_been_party_to_a_lawsuit?') == 'Yes') {
                    $rules['Date_the_lawsuit_was_resolved'] = 'required';
                }

                if ($request->input('In_the_past_10_years_have_you_transferred_any_assets_for_less_t') == 'Yes') {
                    $rules['Date_the_asset_was_transferred'] = 'required';
                }

                if ($request->input('In_the_past_3_year_have_you_transferred_any_real_property') == 'Yes') {
                    $rules['List_the_type_of_property_and_date_of_the_transfer'] = 'required';
                }
                ...
                if($request->file('Business_Bank_Account')){
                    $image = $request->file('Business_Bank_Account');
                    if($image->isValid()){
                        if(!empty($request->input('Business_Bank_Account_old'))){
                            if(file_exists(public_path('/').'/'.$request->input('Business_Bank_Account_old'))){
                                unlink(public_path('/').'/'.$request->input('Business_Bank_Account_old')); 
                            }
                        }
                        $extension = $image->getClientOriginalExtension();
                         $fileName = rand(100,999999).time().'.'.$extension;
                         $image_path = public_path('upload/fq'); 
                        $tt = 'Business_Bank_Account';
                        $request->$tt->move($image_path, $fileName); 
                        $formInput['Business_Bank_Account'] = 'upload/fq/'.$fileName;
                    }
                    unset($formInput['Business_Bank_Account_old']);
                }else{
                    $formInput['Business_Bank_Account'] = $request->input('Business_Bank_Account_old');
                }

                if($request->file('Upload_Most_Recent_Profit_Loss_Statement')){
                    $image = $request->file('Upload_Most_Recent_Profit_Loss_Statement');
                    if($image->isValid()){
                        if(!empty($request->input('Upload_Most_Recent_Profit_Loss_Statement_old'))){
                            if(file_exists(public_path('/').'/'.$request->input('Upload_Most_Recent_Profit_Loss_Statement_old'))){
                                unlink(public_path('/').'/'.$request->input('Upload_Most_Recent_Profit_Loss_Statement_old')); 
                            }
                        }
                        $extension = $image->getClientOriginalExtension();
                         $fileName = rand(100,999999).time().'.'.$extension;
                         $image_path = public_path('upload/fq'); 
                        $tt = 'Upload_Most_Recent_Profit_Loss_Statement';
                        $request->$tt->move($image_path, $fileName); 
                        $formInput['Upload_Most_Recent_Profit_Loss_Statement'] = 'upload/fq/'.$fileName;
                    }
                    unset($formInput['Upload_Most_Recent_Profit_Loss_Statement_old']);
                }else{
                    $formInput['Upload_Most_Recent_Profit_Loss_Statement'] = $request->input('Upload_Most_Recent_Profit_Loss_Statement_old');
                }
                ...
                $data = array(
                      ...
                      "Spouse_Employee_ID" => $request->input('Spouse_Employee_ID'),
                      "Spouse_No_of_Employees" => $request->input('Spouse_No_of_Employees'),
                      "Spouse_Average_Monthly_Payroll" => $request->input('Spouse_Average_Monthly_Payroll'),
                      "Spouse_Type_of_Business" => $request->input('Spouse_Type_of_Business'),
                      "Spouse_Frequency_of_Tax_Deposits" => $request->input('Spouse_Frequency_of_Tax_Deposits'),
                      "Spouse_Business_Website" => $request->input('Spouse_Business_Website'),
                      "Spouse_Employer_Name" => $request->input('Spouse_Employer_Name'),
                      "Spouse_Occupation" => $request->input('Spouse_Occupation'),
                      "Spouse_Employer_Address" => $request->input('Spouse_Employer_Address'),
                      "Spouse_How_Long_with_this_Employer" => $request->input('Spouse_How_Long_with_this_Employer'),
                      "Spouse_Number_of_Exemptions_claimed_on_W4" => $request->input('Spouse_Number_of_Exemptions_claimed_on_W4'),
                      "spStatus" => $request->input('spStatus'),
                      "Spouse_Pay_amount" => $request->input('Spouse_Pay_amount'),

                      "Em_Employer_Name" => $request->input('Em_Employer_Name'),
                      "Em_Occupation" => $request->input('Em_Occupation'),
                      "Em_Employer_Address" => $request->input('Em_Employer_Address'),
                      "Em_How_Long_with_this_Employer" => $request->input('Em_How_Long_with_this_Employer'),
                      "Em_Number_of_Exemptions_claimed_on_W4" => $request->input('Em_Number_of_Exemptions_claimed_on_W4'),
                      "EmspStatus" => $request->input('EmspStatus'),
                      "emspouseamount" => $request->input('emspouseamount'),

                        "Business_Cash_On_Hand" => $request->input('Business_Cash_On_Hand'),
                        "Business_Bank_Account" => $formInput['Business_Bank_Account'],
                        "Are_you_able_to_provide_current_year_Profit_Loss_Statement" => $request->input('Are_you_able_to_provide_current_year_Profit_Loss_Statement'),
                        "Upload_Most_Recent_Profit_Loss_Statement" => $formInput['Upload_Most_Recent_Profit_Loss_Statement'],
                        "Item_Actual_Monthly_Gross_Receipts" => $request->input('Item_Actual_Monthly_Gross_Receipts'),
                        "Gross_Rental_Income" => $request->input('Gross_Rental_Income'),
                        "Interest" => $request->input('Interest'),
                        "Dividends" => $request->input('Dividends'),
                        "Cash" => $request->input('Cash'),
                        "Total_Business_Income" => $request->input('Total_Business_Income'),
                        "Materials_Purchased" => $request->input('Materials_Purchased'),
                        "Inventory_Purchased" => $request->input('Inventory_Purchased'),
                        "Gross_Wages_Salaries" => $request->input('Gross_Wages_Salaries'),
                        "Rent" => $request->input('Rent'),
                        "Supplies" => $request->input('Supplies'),
                        "Business_Expenses" => $request->input('Business_Expenses'),
                        "Utilities_Telephone" => $request->input('Utilities_Telephone'),
                        "Vehicle_Gas_Oil" => $request->input('Vehicle_Gas_Oil'),
                        "Repairs_Maintenance" => $request->input('Repairs_Maintenance'),
                        "Insurance" => $request->input('Insurance'),
                        "Current_Taxes" => $request->input('Current_Taxes'),
                        "Total_Business_Expenses" => $request->input('Total_Business_Expenses'),
                        "Net_Business_Income" => $request->input('Net_Business_Income'),
                );
                ...
                $fq_details = Fq::where('id', $fqid)->first();    
                ...
                $filename =  $name.'_'.$fq_details->created_at.'_fqform.pdf';
				...
				$response9_ = json_decode($response9, true);  

                if($response9_['status'] == 'success'){
                  
                  
                      $dataauth = array();

                      $filename1 =  $name.'_'.$fq_details->created_at.'_signauthfqform.pdf';
                ...
                //   
                      ...
                      $datap["title"] = "Fq-form from [".$clientname."] CASE ID:" .$clientcaseid;
                      $datap["body"] = "Fq-form from [".$clientname."] CASE ID:" .$clientcaseid;
                      ...
                      Mail::send('fqform_mail_template', $datap, function($message)use($datap, $files) {
