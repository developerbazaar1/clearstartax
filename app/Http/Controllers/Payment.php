...
class Payment extends Controller
{
            ...
            if($response['status'] == 'success'){
                $response2 = json_decode($response['data'], true);
                    ...
                    if(strtolower($data['Email']) == strtolower($email)){
                        ...
                        return redirect(route('appointment', $ide  ));
                    }else{
    ...
    curl_close($curl);
    $response1_ = json_decode($response_, true);  
        if(empty($response1_['Message'])){
        
                if($response1_['status'] == 'success'){
                        $response2_ = json_decode($response1_['data'], true);
                        
                        $data['CaseID'] = $response2_['CaseID']; 
                        $data['FirstName'] = $response2_['FirstName']; 
                        $data['LastName'] = $response2_['LastName']; 
                        $data['City'] = $response2_['City']; 
                        $data['State'] = $response2_['State']; 
                        $data['Zip'] = $response2_['Zip']; 
                        $data['Address'] = $response2_['Address']; 
                        $data['AptNo'] = $response2_['AptNo'];
                        $data['CellPhone'] = $response2_['CellPhone'];
                        $data['Email'] = $response2_['Email']; 
                        ...
                        // ------------------------------------
                            ...
                            if($statusid){ 
                                ...
                                // get status
                                    ...
                                    curl_setopt_array($curl, array(
                                      CURLOPT_URL => 'http://client.clearstarttax.com/api/get-status',
                                      CURLOPT_RETURNTRANSFER => true,
                                      CURLOPT_ENCODING => '',
                                      CURLOPT_MAXREDIRS => 10,
                                      CURLOPT_TIMEOUT => 0,
                                    ...
                                    $response_status1 = json_decode($response_status, true);  
                                    if($response_status1['status'] == true){
                                        $all_status = $response_status1['data'];
                                        $statusinfo = collect($all_status)->firstWhere('id', $status_id);
                                    }
                        ...
                        // ====================================
                         ...
                         return view('appointment.index', compact('data', 'settlement_officer', 'status_id', 'statusinfo', 'status_for_appointment'));
  ...
  public function get_available_slots_without45(Request $request)
    {
    ...
    public function get_available_slots_7_3_24_old(Request $request)
    {
                ...
                if ($timeSlots[0] === 'Available time slots in PST') {
                    ...
                    if($timeSlots[0] == 'All slots are booked for this date'){
                        ...
                        foreach ($timeSlots as $slot) {
                            ...
                            if ($slotTime->gte($currentHourMinute) && $slotTime->gte($currentHourMinutePlus45)) { // "gte" means greater than or equal to
                                $timeSlots_arr[] = $slot;
                            }
                        }
                    }
