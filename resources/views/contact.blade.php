...
<div class="app-title">
	...
	<div class="user-dashboard-welcome-d-image d-none-sm">
		...
		<img class="dashboardtop-image cnt-top" src="{{ URL::asset('assets/img/contact.png') }}">
	</div>
</div>
		...
		<div class="tile-x">
			...
			<div class="cnt-form">
				@if($message = Session::get('success'))
                    ...
                    <div class="alert m-3 p-2 px-5 m-px4 alert-success alert-dismissible" role="alert">
                      {{ $message }}
                ...
                @if($message = Session::get('error'))
                    ...
                    <div class="alert m-3 p-2 alert-danger alert-dismissible" role="alert">
                      {{ $message }}
                      <button type="button" class="float-right btn-close" data-dismiss="alert" aria-label="Close"><i class="fa fa-close"></i></button>
                    </div>
                @endif
				<form action="{{route('submit-contact')}}" method="POST" enctype="multipart/form-data">
					...
					<div class="row justify-content-center p-4">
						...
						<!--		<input type="text" class="form-control" id="name"  value="@php if(!empty(Auth::user()->name)){ echo Auth::user()->name; } @endphp" name="name" placeholder="Enter full name" required="">-->
						<!--	</div>-->
						<!--</div>-->
						<!-- :: input 02 -->
						<!--<div class="col-lg-6 col-sm-6 col-md-6 col-xs-12">-->
						...
						<!--		<input type="email" class="form-control" id="email" value="@php if(!empty(Auth::user()->email)){ echo Auth::user()->email; } @endphp" name="email" placeholder="Enter email address" required="">-->
						<!--	</div>-->
						<!--</div>-->
						<input type="hidden" class="form-control" id="name"  value="@php if(!empty(Auth::user()->name)){ echo Auth::user()->name; } @endphp" name="name" placeholder="Enter full name" required="">
						<input type="hidden" class="form-control" id="email" value="@php if(!empty(Auth::user()->email)){ echo Auth::user()->email; } @endphp" name="email" placeholder="Enter email address" required="">
						...
						<div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
							<div class="form-group">
								...
								<textarea class="form-control p-10"  required="" placeholder="Write message" id="message" name="message" rows="3"></textarea>
							</div>
						</div>
						
						<div class="col-lg-12 col-sm-12 col-md-12 col-xs-12">
							<div class="form-group">
								<label class="form-head" for="exampletext"> Upload document : </label>
								<input type="file" name="attachment" class="dropify1"  >
							</div>
						</div>
			...
			<!-- :: info text -->
			<div class="contact-call-us text-center pb-3">
				<p class="mb-1">Need To Call Us?:</p>
				<div class="flex-container justify-content-center  d-lg-flex d-sm-block">
					<div>
						<strong>Contact Hours :</strong> 8:00 AM - 5:00 PM (PST)
					</div>
				
					<div class="mx-1 mb-mx-0">
						<strong>Phone :</strong>
						...
						<a href="tel:888-235-0004">888-235-0004</a>
					</div>
				</div>
			</div>
			<!--:: info call end here -->
